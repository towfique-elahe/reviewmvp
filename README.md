...
- Fixed: Write a review page where logged-in users with multiple suggested courses now have their most recently added course auto-selected, pending courses are shown before published ones in the dropdown, and mobile layout problems with course selection have been resolved.

## V1.2.9

- Fixed: Incorrect password error message on the login page pointing to the reset password page.
- Updated: Replaced all site checkboxes with the new custom checkbox design.
...
- Updated: Add review page, preserves guest drafts (even after LinkedIn sign-in), enforces one review per user per course, awards badges based on approved review count, and restricts each LinkedIn profile to a single account.
